...
- 免打扰模式默认开启；关闭后，中局解说仍只选择稀疏且有情绪价值的事件，并受普通/关键冷却与 30 秒用户聊天静默窗约束。
