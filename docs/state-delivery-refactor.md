...
开场/重连/结算回应是数据共享开启时的固有行为，没有独立开关；免打扰默认开启，只抑制中局主动陪伴。生命周期按来源 epoch、局数和阶段去重，不受中局事件冷却与聊天静默窗影响；bootstrap 只产生 resumed，不伪装为新局开始。现有事件优先级、普通/关键冷却和用户聊天静默窗只在免打扰关闭时用于中局主动陪伴，同时保留以下交付约束：
